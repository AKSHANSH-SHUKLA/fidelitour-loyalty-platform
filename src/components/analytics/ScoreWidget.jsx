  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <svg viewBox="0 0 64 64" width="56" height="56" aria-hidden="true">
        ...
        <circle cx="32" cy="32" r={radius} fill="none" stroke="hsl(230 32% 18%)" strokeWidth="6" />
        <circle
      ...
      <div style={{ fontSize: 11, color: 'hsl(228 14% 81%)', lineHeight: 1.45 }}>
        {delta != null && isFinite(delta) ? (
          <div style={{ color: delta >= 0 ? 'hsl(160 84% 60%)' : 'hsl(351 95% 71%)' }}>
            {delta >= 0 ? '+' : '−'}{Math.abs(delta)} vs sem. dernière
          </div>
        ) : <div style={{ color: 'hsl(228 11% 60%)' }}>—</div>}
        <div style={{ color: 'hsl(228 11% 60%)' }} className="av2-num">{clamped} / {denominator}</div>
      </div>
    </div>
  );
};
